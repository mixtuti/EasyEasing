...
namespace EasyEasing
{
    public static class EasingFunctions
    {
        ...
        {
            t /= total / 2;
            if (t < 1) return (end - begin) / 2 * t * t + begin;
            t--;
        ...
        {
            t /= total / 2;
            if (t < 1) return (end - begin) / 2 * t * t * t + begin;
            t -= 2;
            return (end - begin) / 2 * (t * t * t + 2) + begin;
        }
        #endregion
        ...
        {
            t /= total - 1;
            return -(end - begin) * (t * t * t * t - 1) + begin;
        }

        public static float EaseInOutQuart(float t, float total, float begin, float end)
        {
            t /= total / 2;
            if (t < 1) return (end - begin) / 2 * t * t * t * t + begin;
            t -= 2;
            return -(end - begin) / 2 * (t * t * t * t - 2) + begin;
        }
        #endregion
        ...
        {
            t /= total - 1;
            return (end - begin) * (t * t * t * t * t + 1) + begin;
        }

        public static float EaseInOutQuint(float t, float total, float begin, float end)
        {
            t /= total / 2;
            if (t < 1) return (end - begin) / 2 * t * t * t * t * t + begin;
            t -= 2;
            return (end - begin) / 2 * (t * t * t * t * t + 2) + begin;
        }
        #endregion
